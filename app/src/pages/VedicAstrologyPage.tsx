...
function DeepSectionContent({ section }: { section: VedicReport['sections'][number] }) {
  ...
  return <div className="mt-6 space-y-5"><div className="grid gap-4 md:grid-cols-2">{section.analysisBlocks?.map((block, index) => <section key={`${block.label}-${index}`} className={`${index % 3 === 0 ? 'md:col-span-2' : ''} rounded-2xl border border-violet-300/15 bg-white/[0.025] p-5`}><h4 className="font-serif text-lg text-fuchsia-100">{block.label}</h4><p className="mt-2 leading-7 text-white/68">{block.content}</p></section>)}</div>{section.depth && <section className="grid gap-3 md:grid-cols-3"><TransitionCard label="你看到的表面" text={section.depth.surface} /><TransitionCard label="盤裡更深的原因" text={section.depth.deeperCause} /><TransitionCard label="如果持續不改" text={section.depth.unchangedCost} /></section>}{section.coreTension && <section className="rounded-2xl border border-amber-300/20 bg-amber-300/5 p-5"><h4 className="font-serif text-xl text-amber-100">你內在最大的矛盾</h4><p className="mt-3 text-white/70">一方面：{section.coreTension.sideA}</p><p className="mt-2 text-white/70">另一方面：{section.coreTension.sideB}</p><p className="mt-3 leading-7 text-white/60">現實影響：{section.coreTension.realLifeEffect}</p><p className="mt-2 leading-7 text-amber-50/75">整合方式：{section.coreTension.integrationAdvice}</p></section>}{section.adjustments?.length ? <section className="space-y-3"><h4 className="font-serif text-xl text-cyan-100">對應命盤模式的調整方法</h4>{section.adjustments.map((item, index) => <div key={`${item.problem}-${index}`} className="rounded-xl border border-cyan-300/15 bg-cyan-400/5 p-4"><p className="font-medium text-white/75">{item.problem}</p><p className="mt-2 text-sm text-white/55">星盤原因：{item.astrologicalCause}</p><p className="mt-1 text-sm text-white/55">生活影響：{item.realLifeEffect}</p><p className="mt-2 text-sm text-cyan-100/80">建議：{item.action}</p></div>)}</section> : null}{section.reasoningBasis?.length ? <details className="rounded-xl border border-white/10 px-4 py-3"><summary className="cursor-pointer text-violet-100/70">為什麼會這樣？・判讀依據：{confidence}</summary><div className="mt-3 space-y-3">{section.reasoningBasis.map((item, index) => <div key={`${item.factor}-${index}`} className="text-sm leading-6 text-white/55"><strong className="text-violet-100/75">{item.factor}</strong><p>{item.meaning}</p><p>{item.contribution}</p></div>)}</div></details> : null}</div>;
}
...
function ForecastCard({ period }: { period: NonNullable<VedicReport['sections'][number]['timeline']>[number] }) {
  ...
  return <article className="rounded-2xl border border-fuchsia-300/20 bg-slate-950/50 p-5 sm:p-6">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="rounded-xl bg-white/[0.035] p-4"><p className="font-medium text-fuchsia-100">為什麼？・判讀依據：{interpretation.confidence === 'high' ? '較強' : interpretation.confidence === 'medium' ? '中等' : '有限'}</p><p className="mt-2 text-sm leading-6 text-white/60">{interpretation.why}</p></div><div className="rounded-xl border border-amber-300/15 bg-amber-300/5 p-4"><p className="font-medium text-amber-100">一句話提醒</p><p className="mt-2 text-sm leading-6 text-white/70">{interpretation.keyMessage}</p></div></div>
  </article>;
}
